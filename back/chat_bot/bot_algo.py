...
class TradingBot:
    ...
    def fetch_historical_data(self, start_date, end_date):
        ...
        data['EMA_50'] = self.indicators.ema(data['Close'], 50)
    ...
    def evaluate_trading_strategy(self):
        ...
        self.df['SMA_20'] = self.indicators.sma(self.df['price'], 20)
        self.df['SMA_50'] = self.indicators.sma(self.df['price'], 20)
        self.df['EMA_20'] = self.indicators.ema(self.df['price'], 20) 
        self.df['EMA_50'] = self.indicators.ema(self.df['price'], 50)
        self.df['RSI'] = self.indicators.rsi(self.df['price'], 14)
        macd, signal, hist = self.indicators.macd(self.df['price'])
        self.df['MACD'] = macd
