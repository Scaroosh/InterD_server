...
const CryptocurrencySearchOutput = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://127.0.0.1:8080/crypto/analyze', { crypto_symbol: cryptoSymbol });
        ...
        <div className={classes.container}>
            <form onSubmit={handleSubmit} className={classes.form}>
                <input
                    ...
                    required
                />
                <button type="submit" className={classes.button}>Analyze</button>
            </form>
            {loading && (
            ...
            {result && !loading && (
                <div className={classes.result}>
                    ...
                    <JSONPretty data={result} style={{ whiteSpace: 'pre-wrap' }}></JSONPretty>
                </div>
            )}
        </div>
    );
